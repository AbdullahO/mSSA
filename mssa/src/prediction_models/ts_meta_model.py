...
class TSMM(object):
    ...
    def update_model(self, NewEntries):
        ...
        if current_no_models == updated_no_models:
            ...
            last_model_size = (
                self.models[updated_no_models - 1].M
                * self.models[updated_no_models - 1].N
            )
            ...
            if current_no_models > 0:
                ...
                FillElements = (
                    int((self.T / 2 - fillFactor)) * (fillFactor > 0)
                ) // self.no_ts
                if FillElements > 0:
            ...
            for i in range(
                updated_no_models - current_no_models + (current_no_models == 0)
            ):
                if SkipNext:
                    SkipNext = False
                    continue
                ...
                if len(self.models) == 0:
                    ...
                    self.updateTS(
                        NewEntries[
                            i
                            * int((self.T // self.no_ts) / 2) : (i + 1)
                            * int((self.T // self.no_ts) / 2),
                            :,
                        ]
                    )
                    self.fitModels()
    ...
    def updateTS(self, NewEntries):
        ...
        if N > self.T / 2 and len(self.models) > 1:
            raise Exception(
                "TimeSeries should be updated before T/2 values are assigned"
            )
        ...
        if self.TimeSeriesIndex == N or self.TimeSeries is None:
            ...
            self.TimeSeries[: num_ts_obs - num_new_rows, :] = self.TimeSeries[
                -num_ts_obs + num_new_rows :, :
            ]
    ...
    def fitModels(self):
        ...
        if lenEntriesSinceLastUpdate > self.T and ModelIndex != 0:
            print(
                ...
                self.MUpdateIndex,
                [(m.N, m.M, m.start) for m in self.models.values()],
            )
        ...
        if ModelIndex not in self.models:
            ...
            initEntries = self.TimeSeries[
                int(int(self.T / 2) - self.TimeSeriesIndex % (self.T / 2))
                // self.no_ts :,
                :,
            ]
            start = (
                self.TimeSeriesIndex
                - self.TimeSeriesIndex % int(self.T / 2)
                - int(self.T / 2)
            )
            ...
            if M < self.no_ts:
                raise Exception(
                    "Number of columns in the matrix (%s) is less than the number of time series (%s)"
                    % (M, self.no_ts)
                )
            if M % self.no_ts != 0:
                M -= M % self.no_ts
            ...
            self.models[ModelIndex] = SVDModel(
                "t1",
                self.kSingularValuesToKeep,
                N,
                M,
                start=int(start),
                SSVT=self.SSVT,
            ...
            flattened_obs = flattened_obs[
                :, np.arange(M_ts * self.no_ts).reshape([self.no_ts, M_ts]).flatten("F")
            ]
            self.models[ModelIndex].fit(
                pd.DataFrame(data={"t1": flattened_obs.flatten("F")})
            )
        ...
        if (
            TSlength <= self.TimeSeries.size
            and (
                float(lenEntriesSinceCons) / (self.ReconIndex - Model.start)
                >= self.gamma
            )
        ) or (
            self.TimeSeriesIndex % (self.T / 2) == 0
        ):  # condition to recompute SVD
            if self.persist_L:
                N = self.L
            else:
            ...
            flattened_obs = flattened_obs[
                :, np.arange(M).reshape([self.no_ts, M_ts]).flatten("F")
            ]
            ...
            if self.TimeSeriesIndex - ModelLength < N * self.no_ts:
                ...
                NewEntries = self.TimeSeries[
                    -(self.TimeSeriesIndex - ModelLength) // self.no_ts :, :
                ]
                if self.normalize:
                    NewEntries = NewEntries - Model.norm_mean
                    NewEntries = NewEntries / Model.norm_std
                num_new_columns = self.no_ts * ((NewEntries[:, 0]).size // N)
                flattened_obs = NewEntries[
                    : (num_new_columns * N) // self.no_ts, :
                ].reshape([N, num_new_columns], order="F")
                flattened_obs = flattened_obs[
                    :,
                    np.arange(num_new_columns)
                    .reshape([self.no_ts, num_new_columns // self.no_ts])
                    .flatten("F"),
                ]
        ...
        if range_ or index is None:
            ...
            for Model in models.values():
                ...
                if x1 <= y2 and y1 <= x2:
                    ...
                    denoised_columns_swapped = denoised_matrix[
                        :,
                        np.arange(M)
                        .reshape([M // self.no_ts, self.no_ts])
                        .flatten("F"),
                    ]
                    denoised_ts = denoised_columns_swapped.reshape(
                        [denoised_columns_swapped.size // self.no_ts, self.no_ts],
                        order="F",
                    )
                    denoised_index = RIndex - x1 // self.no_ts
                    denoised[RIndexS[0] : RIndexS[1], :] += (
                        denoised_ts[denoised_index[0] : denoised_index[1], :]
                        * Model.norm_std
                        + Model.norm_mean
                    )
            ...
            for ModelNumber in np.unique(models):
                ...
                denoised[updatedIndices] += Model.denoisedTS(
                    index[updatedIndices] - x1, range_
                )
    ...
    def _predict(
        self, index=None, method="average", NoModels=None, dataPoints=None, models=None
    ):
        ...
        if NoModels is None or NoModels > n or NoModels < 1:
            NoModels = n
